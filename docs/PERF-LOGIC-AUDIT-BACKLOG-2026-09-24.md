...
| WON'T DO（2026-09-24 决策：按设计接受） | 明文凭据 | `local-server.mjs:116-150`（`~/.waveforge/qq-cookie.txt`）、各平台 localStorage | 凭据明文落盘 | **不做**（理由见 §9 决策记录） | — |
...
### 决策记录：明文凭据（2026-09-24，WON'T DO）

- **结论**：不引入系统凭据库/加密，也不改文件 ACL。
- **理由**：① 不影响功能——这只是存储形态，应用行为与之一致；② 加固本身有自伤风险：Windows 下给 `~/.waveforge/*.txt` 配 ACL，一旦配置错误或应用以不同上下文运行，应用**读不到自己写的文件**（登录态直接失效）；DPAPI/凭据库在用户改密、换机器、还原配置档案、漫游配置之后会**解不开**，表现为静默登出；③ 威胁模型只覆盖「同机同身份的程序」——而桌面应用的信任边界本来就是「以当前 OS 用户身份运行的东西可信」。
