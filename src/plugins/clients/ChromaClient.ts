...
export function normalizeChromaSettings(value: unknown): ChromaSettings {
  ...
  const normalized: ChromaSettings = {
    ...
    brightness: clamp(input.brightness, defaults.brightness, 0.4, 2),
    ...
    size: clamp(input.size, defaults.size, 1, 10),
    spectrumMirrored: input.spectrumMirrored === true,
    colorRotationSpeed: clamp(
      input.colorRotationSpeed,
      defaults.colorRotationSpeed,
      0,
      2,
    ),
...
function createClient() {
  ...
  const pushResult = () => {
    ...
    set({ preview: result.frames, lastFrameAt: Date.now() });
  ...
  return {
    ...
    deactivate: async () => {
      ...
      engine.reset();
      releaseSent = false;
      set({ active: false, preview: emptyFrames(), lastFrameAt: null });
      if (window.electron?.chroma)
        set({ status: await window.electron.chroma.deactivate() });
    },
