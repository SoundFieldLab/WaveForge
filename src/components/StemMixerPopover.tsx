...
export function StemMixerPopover({ control, accentColor, theme }: StemMixerPopoverProps) {
  ...
  return (
    <div ref={rootRef} className="relative flex items-center">
      <motion.button
        ...
        disabled={disabled}
        className={`relative rounded-full p-2 transition-colors disabled:cursor-not-allowed disabled:opacity-35 ${dark ? 'hover:bg-white/10' : 'hover:bg-black/10'}`}
        title={control.locked ? 'AutoMix 过渡期间暂不可调整分轨' : control.reason || '人声与乐器调节'}
        aria-label="人声与乐器调节"
      >
        <AudioLines className="h-4 w-4" style={{ color: control.active ? accentColor : muted }} />
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute bottom-full right-0 z-[190] mb-3 w-[300px] rounded-2xl border p-3 shadow-2xl"
