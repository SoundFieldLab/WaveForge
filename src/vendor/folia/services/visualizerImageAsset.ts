// WaveForge 桩实现：上游使用 IndexedDB（services/db.ts）持久化可视化器图片资产，
// 该闭包依赖整个 folia 应用外壳，这里改为会话内内存存储，接口签名与上游完全一致。
...
const memoryStore = new Map<string, StoredVisualizerImageAsset>();
...
export const getStoredVisualizerImageAsset = async <T extends StoredVisualizerImageAsset>(key: string): Promise<T | null> => {
    ...
    if (!stored?.blob || !(stored.blob instanceof Blob) || typeof stored.name !== 'string') return null
    return stored as T
};
