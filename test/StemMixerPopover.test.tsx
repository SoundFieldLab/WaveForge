...
describe('StemMixerPopover', () => {
  ...
  it('uses the immersive trigger and opens the shared popover to the left', () => {
    render(
      <StemMixerPopover
        control={control()}
        accentColor="#3b82f6"
    ...
    const trigger = screen.getByRole('button', { name: '人声与乐器调节' })
    expect(trigger.className).toContain('p-3')
    expect(trigger.parentElement?.dataset.stemMixerVariant).toBe('immersive')
    fireEvent.click(trigger)
    expect(screen.getByTestId('stem-mixer-popover').className).toContain('right-full')
    expect(screen.getByRole('slider', { name: '人声音量' })).toBeTruthy()
  })
