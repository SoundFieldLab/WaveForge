...
function plan(): TransitionPlan {
  return {
    ...
    v2: {
      ...
      beatProvider: 'beat_this',
      folia: {
        beatProvider: 'beat_this',
        maxStemWindowSeconds: 40,
        sourceDownbeats: [90, 95, 100],
        targetDownbeats: [10, 15, 20],
      },
      stemRequirement: {
        source: { role: 'tail', startTime: 60, duration: 40 },
        target: { role: 'head', startTime: 10, duration: 40 },
...
describe('refineTransitionWithStems', () => {
  ...
  it('模型可用时并行分离两侧并生成 stem choreography 和稳定指纹', async () => {
    ...
    expect(separate.mock.calls[0][0].duration).toBe(40)
    expect(separate.mock.calls[1][0].duration).toBe(40)
    expect(result?.plan.v2?.stemChoreography?.drumSwap.time).toBe(5)
    expect(result?.plan.v2?.stemChoreography?.drumSwap.alignment).toBe('downbeat')
  })
