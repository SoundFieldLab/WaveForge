import { createLogBandEdges, rebinSpectrumMeanPower } from "../../rgb/rgbSpectrum";
import { applyChromaTopologyMask, getChromaDeviceTopology } from "./chromaTopology";
import {
  CHROMA_VISUALIZER_HEIGHT,
  CHROMA_VISUALIZER_WIDTH,
  createChromaVisualizerField,
  projectVisualizerCoverage,
  projectVisualizerField,
  type ChromaVisualizerField,
} from "./chromaVisualizerField";
import {
...
export function resampleSpectrum(
  ...
  const redistributed = rebinSpectrumMeanPower(power, length, createLogBandEdges(input.length));
...
function frequencyPosition(
  ...
  return mirrored && direction === "ltr"
    ? Math.abs(directed * 2 - 1)
    : directed;
}

function visualScale(level: number, size: number): number {
  const normalizedSize = clamp(size, 1, 10);
  const gainDb = normalizedSize <= 5
    ? -18 + (normalizedSize - 1) * 4.5
    : (normalizedSize - 5) * 3;
  return clamp(level * 10 ** (gainDb / 20));
}
...
function composeSample(
  ...
  const backgroundCoverage = clamp(background.coverage);
  const litForeground =
    beatFlash > 0
...
const SPECTRUM_STYLES = new Set<KeyboardChromaSettings["style"]>([
  "bars",
  "spectrum-cycle",
  "spectrum-static",
  "spectrum-gradient",
...
interface ChromaVisualizerLayers {
  foreground: ChromaVisualizerField;
  background: ChromaVisualizerField;
  composite: ChromaVisualizerField;
}

function createSpectrumVisualizerLayers(
  settings: KeyboardChromaSettings,
  audio: NormalizedAudio,
  now: number,
  config: ChromaSettings,
): ChromaVisualizerLayers {
  const foreground = createChromaVisualizerField();
  const background = createChromaVisualizerField();
  const composite = createChromaVisualizerField();
  const spectrum = resampleSpectrum(audio.spectrum, CHROMA_VISUALIZER_WIDTH);
  const fieldRows = CHROMA_VISUALIZER_HEIGHT - 1;
  const foregroundSettings: KeyboardChromaSettings = {
    ...settings,
    direction: config.foregroundDirection,
    theme: settings.style === "static" ? "custom" : config.foregroundTheme,
    customColors: settings.style === "static"
      ? [config.foregroundStaticColor, config.foregroundStaticColor]
      : config.foregroundGradient,
    beatFlash: config.foregroundBeatFlash,
  };
  const beatFlash = config.foregroundBeatFlash
    ? clamp(audio.beat * 0.7 + audio.accent * 0.3) * 0.65
    : 0;
  const audioDrive = clamp(Math.max(
    audio.overall,
    audio.bass,
    audio.mid,
    audio.high,
  ...
  const stripLevel = clamp(
    audio.overall * 0.5 + audio.high * 0.2 + audio.beat * 0.2 + audio.accent * 0.1,
  );

  for (let column = 0; column < CHROMA_VISUALIZER_WIDTH; column += 1) {
    const foregroundPosition = frequencyPosition(
      column,
      CHROMA_VISUALIZER_WIDTH,
      config.foregroundDirection,
      config.foregroundMirrored,
    );
    const backgroundPosition = directionalPosition(
      column,
      CHROMA_VISUALIZER_WIDTH,
      config.backgroundDirection,
    );
    const sourceIndex = Math.round(foregroundPosition * (spectrum.length - 1));
    const energy = clamp(spectrum[sourceIndex] ?? 0);
    let foregroundColor = config.foregroundStaticColor;
    if (settings.style === "spectrum-gradient" || settings.style === "bars") {
      foregroundColor = interpolateColor(
        config.foregroundGradient[0],
        config.foregroundGradient[1],
        foregroundPosition,
      );
    } else if (settings.style === "spectrum-cycle") {
      foregroundColor = hsvColor(
        foregroundPosition + (now / 5000) * config.foregroundAnimationSpeed,
      );
    }
    const spectrumStyle = SPECTRUM_STYLES.has(settings.style);
    const stripSample = spectrumStyle
      ? { color: foregroundColor, coverage: foregroundPosition <= stripLevel ? 1 : 0 }
      : keyboardForeground(
          foregroundSettings,
          audio,
          0,
          ...
          5,
          config.foregroundMirrored,
          config.foregroundAnimationSpeed,
          CHROMA_VISUALIZER_HEIGHT - 1,
          CHROMA_VISUALIZER_WIDTH,
          false,
        );
    const stripColor = beatFlash > 0
      ? interpolateColor(stripSample.color, "#ffffff", beatFlash)
      : stripSample.color;
    const stripCoverage = clamp(
      (clamp(stripSample.coverage) * audioDrive + beatFlash * 0.35) * config.brightness,
    );
    foreground.colors[column] = packBgr(stripColor, 1);
    foreground.coverage[column] = stripCoverage;

    let stripBackground: ForegroundSample = { color: "#000000", coverage: 0 };
    if (config.backgroundEffect !== "off") {
      stripBackground = backgroundSample(
        config.backgroundEffect,
        {
          theme: "custom",
          customColors: config.backgroundGradient,
          background: config.backgroundStaticColor,
          beatFlash: false,
          direction: config.backgroundDirection,
        },
        backgroundPosition,
        0,
        audio,
        now * Math.max(0.05, config.backgroundAnimationSpeed),
        config.backgroundBrightness,
        config.reactiveBackground,
      );
      background.colors[column] = packBgr(stripBackground.color, 1);
      background.coverage[column] = stripBackground.coverage;
    }
    composite.colors[column] = packBgr(
      composeSample(
        stripBackground,
        { color: stripColor, coverage: stripCoverage },
        0,
      ),
      1,
    );
    composite.coverage[column] = clamp(stripCoverage + stripBackground.coverage * (1 - stripCoverage));

    for (let row = 1; row < CHROMA_VISUALIZER_HEIGHT; row += 1) {
      const index = row * CHROMA_VISUALIZER_WIDTH + column;
      const levelFromBottom = CHROMA_VISUALIZER_HEIGHT - 1 - row;
      const foregroundSample = spectrumStyle
        ? {
            color: foregroundColor,
            coverage: clamp(energy * fieldRows - levelFromBottom),
          }
        : keyboardForeground(
            foregroundSettings,
            audio,
      ...
      const litForeground = beatFlash > 0
        ? interpolateColor(foregroundSample.color, "#ffffff", beatFlash)
        : foregroundSample.color;
      const foregroundCoverage = clamp(
        (foregroundSample.coverage + beatFlash * 0.35) * audioDrive * config.brightness,
      );
      foreground.colors[index] = packBgr(litForeground, 1);
      foreground.coverage[index] = foregroundCoverage;
      let backgroundValue: ForegroundSample = { color: "#000000", coverage: 0 };
      if (config.backgroundEffect !== "off") {
        backgroundValue = backgroundSample(
          config.backgroundEffect,
          {
          ...
          backgroundPosition,
          (row - 1) / Math.max(1, fieldRows - 1),
          audio,
          now * Math.max(0.05, config.backgroundAnimationSpeed),
          config.backgroundBrightness,
          config.reactiveBackground,
        );
        background.colors[index] = packBgr(backgroundValue.color, 1);
        background.coverage[index] = backgroundValue.coverage;
      }
      composite.colors[index] = packBgr(
        composeSample(
          backgroundValue,
          { color: litForeground, coverage: foregroundCoverage },
          0,
        ),
        1,
      );
      composite.coverage[index] = clamp(foregroundCoverage + backgroundValue.coverage * (1 - foregroundCoverage));
    }
  }
  return { foreground, background, composite };
}

function scalePackedBgr(color: number, scale: number): number {
  const amount = clamp(scale);
  const red = Math.round((color & 0xff) * amount);
  const green = Math.round(((color >>> 8) & 0xff) * amount);
  const blue = Math.round(((color >>> 16) & 0xff) * amount);
  return ((blue << 16) | (green << 8) | red) >>> 0;
}

function blendPackedBgr(background: number, foreground: number, coverage: number): number {
  const amount = clamp(coverage);
  const blend = (shift: number) => Math.round(
    ((background >>> shift) & 0xff) * (1 - amount) +
      ((foreground >>> shift) & 0xff) * amount,
  );
  return ((blend(16) << 16) | (blend(8) << 8) | blend(0)) >>> 0;
}

function projectSpectrumFrames(
  layers: ChromaVisualizerLayers,
  settings: ChromaSettings,
  hardwareDevices: ChromaRenderOptions["hardwareDevices"],
): ChromaFrames {
  const frames = {} as ChromaFrames;
  for (const device of CHROMA_DEVICE_TYPES) {
    const topology = getChromaDeviceTopology(device, hardwareDevices);
    if (!settings[device].enabled) {
      frames[device] = new Uint32Array(topology.rows * topology.columns);
      continue;
    }
    const projectedForeground = projectVisualizerField(device, layers.foreground, topology, settings.size);
    const projectedForegroundCoverage = projectVisualizerCoverage(device, layers.foreground, topology, settings.size);
    const projectedBackground = projectVisualizerField(device, layers.background, topology, 5);
    const projectedBackgroundCoverage = projectVisualizerCoverage(device, layers.background, topology, 5);
    const projected = new Uint32Array(projectedForeground.length);
    for (let index = 0; index < projected.length; index += 1) {
      const backgroundColor = scalePackedBgr(projectedBackground[index], projectedBackgroundCoverage[index]);
      projected[index] = blendPackedBgr(
        backgroundColor,
        projectedForeground[index],
        projectedForegroundCoverage[index],
      );
    }
    const intensity = clamp(settings[device].intensity ?? 1, 0, 2);
    if (intensity !== 1) {
      for (let index = 0; index < projected.length; index += 1) {
        const color = projected[index];
        const red = Math.round(clamp((color & 0xff) * intensity, 0, 255));
        const green = Math.round(clamp(((color >>> 8) & 0xff) * intensity, 0, 255));
        const blue = Math.round(clamp(((color >>> 16) & 0xff) * intensity, 0, 255));
        projected[index] = ((blue << 16) | (green << 8) | red) >>> 0;
      }
    }
    frames[device] = projected;
  }
  return frames;
}
...
function spectrumForeground(
  ...
  const coverage = clamp(energy * rows - levelFromBottom);
...
function keyboardForeground(
  ...
  mirrored: boolean,
  rotationSpeed: number,
  canvasRows = CHROMA_DEVICE_METADATA.keyboard.rows,
  canvasColumns = CHROMA_DEVICE_METADATA.keyboard.columns,
  applyProjectionScale = true,
): ForegroundSample {
  ...
  const energy = applyProjectionScale
    ? visualScale(groupedEnergy(audio.spectrum, position, 1), size)
    : groupedEnergy(audio.spectrum, position, 1);
...
function idleFrames(
  ...
  for (const device of CHROMA_DEVICE_TYPES) {
    ...
    if (settings.backgroundEffect !== "off" && settings.idleMode !== "off" && deviceSettings.enabled) {
      const color = settings.backgroundEffect === "static" || settings.backgroundEffect === "breath"
        ? settings.backgroundStaticColor
        : interpolateColor(settings.backgroundGradient[0], settings.backgroundGradient[1], 0.45);
      frame.fill(
        packBgr(
          color,
          settings.backgroundBrightness *
            level *
            clamp(deviceSettings.intensity ?? 1, 0, 1),
        ),
      );
    }
    frames[device] = applyChromaTopologyMask(
      frame,
      getChromaDeviceTopology(device, hardwareDevices),
    );
  }
  return frames;
}
    ...
    render(
      ...
      if (idle) {
        if (settings.idleMode === "release")
          return { action: "release", frames: releasedFrames(), foregroundField: null, backgroundField: null, visualizerField: null };
        return { action: "frame", frames: idleFrames(settings, now, options.hardwareDevices), foregroundField: null, backgroundField: null, visualizerField: null };
      }
      ...
      const effectiveSettings: ChromaSettings = {
        ...settings,
        brightness: clamp(settings.brightness, 0, 1),
        decay: clamp(settings.decay ?? 5, 1, 10),
        size: clamp(settings.size ?? 5, 1, 10),
        colorRotationSpeed: clamp(settings.foregroundAnimationSpeed, 0, 2),
        backgroundEffect: settings.backgroundEffect ?? "off",
        backgroundBrightness: clamp(
          settings.backgroundBrightness ?? 0.3,
          0.01,
          1,
        ),
        reactiveBackground: settings.backgroundEffect !== "off" && settings.reactiveBackground === true,
        spectrumMirrored: settings.foregroundMirrored === true,
      };
      const foregroundSettings: KeyboardChromaSettings = {
        ...settings.keyboard,
        direction: settings.foregroundDirection,
        theme: settings.foregroundTheme,
        customColors: settings.foregroundGradient,
        beatFlash: settings.foregroundBeatFlash,
      };
      const layers = createSpectrumVisualizerLayers(
        foregroundSettings,
        smoothed,
        now,
        effectiveSettings,
      ...
      return {
        action: "frame",
        frames: projectSpectrumFrames(
          layers,
          settings,
          options.hardwareDevices,
        ),
        foregroundField: layers.foreground,
        backgroundField: layers.background,
        visualizerField: layers.composite,
      };
    },
    reset() {
      lastNow = null;
