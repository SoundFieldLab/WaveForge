...
// Reading-alignment (鍥炴): a SOFT reading aid inside the shot system, never a lock. The camera's
...
const CameraRig: React.FC<CameraRigProps> = ({
    ...
    useFrame((state, delta) => {
        ...
        // 鈹€鈹€ Cinematic transition flight (flavours the normal follow, never replaces it) 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
        ...
        // Reading-alignment (鍥炴): ease the camera's orientation toward the TEXT PLANE's orientation,
        ...
        // roll 只转移 35%：相机对正文平面，但不全盘继承行的倾角——
        // 原先全量继承让每一行都变成左斜/右斜的荷兰角（用户反馈"不是左斜就是右斜"），
        // 部分转移后地平线基本保持水平，行的舞台倾角留在画面里做构图，镜头多样性交给 shot 语言
        if (placement.roll !== 0) _alignQuat.multiply(_tiltQuat.setFromAxisAngle(_AXIS_Z, placement.roll * 0.35));
