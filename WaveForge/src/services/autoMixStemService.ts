  ...
  try {
    ...
    const stemChoreography = planStemTransition({
      ...
      coarsePlan: {
        ...
        sourceDownbeats: plan.v2?.folia?.sourceDownbeats
          ?? plan.sourceBeatTimes?.filter((_, index) => index % 4 === 0),
        targetDownbeats: plan.v2?.folia?.targetDownbeats
          ?? plan.targetBeatTimes?.filter((_, index) => index % 4 === 0),
