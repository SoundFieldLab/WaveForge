...
/**
 * WaveForge 性能适配：把 r3f 渲染循环门控到 60fps。
 * r3f 默认 frameloop="always" 跟随显示器刷新率——120/240Hz 屏上整帧 WebGL 渲染
 * （粒子 + 几何 + MSAA 抗锯齿）成本是 60fps 的 2-4 倍。改 demand 模式 + 60fps 手动
 * invalidate：useFrame 与渲染都按 60fps 执行，所有效果保留、粒子动画肉眼无差。
 */
const SixtyFpsFrameInvalidator: React.FC = () => {
    ...
    useEffect(() => {
        const tick = (now: number) => {
            ...
            if (now - lastRef.current >= 1000 / 60) {
                lastRef.current = now;
                invalidate();
            }
        };
