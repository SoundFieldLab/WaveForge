...
class SignalRgbEffectManager {
  constructor(options = {}) {
    ...
    this.version = String(options.version || '1.0.1')
    ...
    this.state = {
      ...
      effectInstalled: false, effectPath: null, effectVersion: null, effectUpdateAvailable: false, hash: null, effectHash: null,
  ...
  async refreshInstallation() {
    ...
    if (!installations.length) {
      Object.assign(this.state, { effectInstalled: false, effectPath: null, effectVersion: null, effectUpdateAvailable: false, hash: null, effectHash: null, conflict: false, restartRequired: false })
      return null
    }
    const latest = await this.inspectInstallation(installations[0])
    let bundledHash = null
    try { bundledHash = sha256(await this.fs.promises.readFile(this.bundledEffectPath)) } catch { bundledHash = null }
    ...
    Object.assign(this.state, {
      ...
      effectUpdateAvailable: Boolean(latest.owned && bundledHash && (latest.hash !== bundledHash || latest.metadata.version !== this.version)),
  ...
  async refresh() {
    ...
    const [probes, canvasEventAvailable] = await Promise.all([
      Promise.all([
